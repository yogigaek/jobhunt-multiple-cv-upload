...
UserSchema.path('username').validate(value => {
	const ALPHANUMERIC_RE = /^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z0-9]+$/
	return ALPHANUMERIC_RE.test(value)
}, attr => `${attr.value} username harus mengandung minimal satu angka dan satu huruf`)

UserSchema.path('password').validate(value => {
    const PASSWORD_RE = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z]{6,100}$/
    return PASSWORD_RE.test(value)
}, attr => `${attr.value} password harus terdiri dari 6 hingga 100 karakter, minimal satu huruf besar, minimal satu huruf kecil, dan minimal satu angka`)
